...
# Instruksi Database
1. Buat database MySQL/MariaDB dengan nama `webgis-php`.
2. Import salah satu file SQL yang tersedia, disarankan `webgis-php-leveluser.sql`.
3. Pastikan nama database pada `env.php` sama:

```php
$setDb['db_name']='webgis-php';
```

Jika memakai nama database lain, ubah nilai `db_name` di `env.php` dan `env.example.php` agar sesuai.
