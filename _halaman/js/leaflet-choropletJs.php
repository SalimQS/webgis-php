...
<script type="text/javascript">
	...
	foreach ($getKabupaten as $row) {
		$db->where('id_kabupaten', $row->id_kabupaten);
		$db->get('t_hotspot');
		$data[$row->kd_kabupaten] = $db->count;
	}
	...
	info.update = function(props) {
		this._div.innerHTML = '<h4>Hotspost di Kabupaten Tanah Laut</h4>' + (props ?
			'<b>' + props.KABUPATEN + '</b><br />' + HOTSPOT[props.KODE] + ' titik' :
	...
	<?php
		$arrayKabupaten[] = '{
			...
			layer: new L.GeoJSON.AJAX(["assets/unggah/geojson/' . $row->geojson_kabupaten . '"],{
				style: style,
